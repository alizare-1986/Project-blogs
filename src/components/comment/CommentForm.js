...
const sendHandler = () => {
    if(name && email && text) {
      ...
      toast.warn('تمام فیلد ها را پر کن',{
        position:'top-center'
      })
    }
}
if(data ,press) {
    ...
    toast.success('کامنت ارسال شدومنتظر تایید میباشد',{
        position:'top-center'
    })
    setPress(false)
}
    return (
       ...
       <Grid item xs={12} m={2}>
        <Typography component='p' variant='h6' fontWeight={700} color='primary'>کامنت</Typography>
       </Grid>
       <Grid item xs={12} m={2}>
       ...
       <Grid item xs={12} m={2}>
       <TextField label='متن کامنت' variant='outlined' sx={{width:'100%'}} value={text} onChange={(e)=> setText(e.target.value)} multiline rows={4}/>
       </Grid>
       <Grid item xs={12} m={2}>
